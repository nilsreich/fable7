...
<summary>Verständnisfrage: Warum darf der konstante Faktor beim Ableiten einfach stehen bleiben?</summary>

$c \cdot g(x)$ ist der in $y$-Richtung gestreckte Graph von $g$ – und die
Streckung erfasst **alle** $y$-Werte, also auch jedes Steigungsdreieck:
$\Delta y$ wird verdreifacht, $\Delta x$ bleibt. Jede Steigung ist damit
genau $c$-mal so groß wie vorher. Achtung: Das gilt nur für
**konstante** Faktoren – für Produkte wie $x^2(x-3)$ erst
ausmultiplizieren!
...
<details class="verstaendnis">
<summary>Verständnisfrage: $f(x) = x^2$ und $g(x) = x^2 + 7$ haben dieselbe Ableitung $2x$. Wie kann das sein?</summary>

Der Graph von $g$ ist nur der um 7 nach oben geschobene Graph von $f$ –
an der **Steilheit** ändert das nichts, an jeder Stelle sind beide
Tangenten parallel. Die Ableitung „sieht“ Verschiebungen in $y$-Richtung
nicht: Aus $f'$ allein kann man $f$ deshalb nie vollständig
rekonstruieren.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Was misst die zweite Ableitung anschaulich?</summary>

$f''$ ist die Ableitung von $f'$ – sie misst also, wie schnell sich die
**Steigung** ändert. Beim Autofahren: $s(t)$ = Ort, $s'(t)$ =
Geschwindigkeit (Tacho), $s''(t)$ = Beschleunigung (Gaspedal/Bremse).
Ein positives $f''$ heißt: Die Steigung nimmt zu, der Graph macht eine
Linkskurve.
...
a) Finde den Fehler: „$\left(x^3 \cdot x^2\right)' = 3x^2 \cdot 2x = 6x^3$.“
b) Wahr oder falsch? „Wenn $f'(x) = g'(x)$ für alle $x$ gilt, dann ist
$f = g$.“

<details>
<summary>Lösung zu Aufgabe 17</summary>

a) Es gibt (noch) keine Regel „Produkt = Produkt der Ableitungen“! Erst
mit den Potenzgesetzen zusammenfassen: $x^3 \cdot x^2 = x^5$, dann
Potenzregel: $\left(x^5\right)' = 5x^4$. (Der falsche Weg liefert
$6x^3$ – schon der Exponent passt nicht.)

b) **Falsch.** Gleiche Ableitung heißt nur: an jeder Stelle gleiche
Steigung. Die Graphen können parallel übereinander liegen –
$f(x) = x^2$ und $g(x) = x^2 + 7$ haben beide die Ableitung $2x$.
$f$ und $g$ unterscheiden sich höchstens um eine Konstante.
...
<Quiz fragen={[
  ...
  { frage: 'Verständnisfrage: f′(x) = g′(x) für alle x. Was folgt für f und g?',
    antworten: ['f = g', 'f und g unterscheiden sich höchstens um eine Konstante', 'f und g haben dieselben Nullstellen', 'g ist die zweite Ableitung von f'],
    richtig: 1, erklaerung: 'Gleiche Steigung überall heißt: Die Graphen verlaufen parallel – wie x² und x² + 7. Die Verschiebung sieht die Ableitung nicht.' },
  { frage: 'Verständnisfrage: s(t) beschreibt den Ort eines Autos. Was bedeutet s″(t)?',
    antworten: ['Die Geschwindigkeit', 'Den doppelten Weg', 'Die Beschleunigung – die Änderungsrate der Geschwindigkeit', 'Die Fahrzeit'],
    richtig: 2, erklaerung: 's′ ist die Geschwindigkeit (Tacho); deren Änderungsrate s″ spürt man als Gasgeben oder Bremsen.' },
]} />
