...
<summary>Verständnisfrage: Ist $f_t(x) = x^2 + t$ eine Funktion mit zwei Variablen?</summary>

Nein – die Rollen sind verschieden: Erst wird $t$ **festgelegt** (damit
steht fest, *welche* Kurve gemeint ist), dann läuft $x$ als einzige
Variable. Eine Schar ist also ein ganzes Paket aus unendlich vielen
gewöhnlichen Funktionen, ordentlich durchnummeriert mit $t$ – keine
Funktion von zwei Größen gleichzeitig.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Woran erkennst du schon <em>während</em> der Rechnung, dass eine Fallunterscheidung nötig wird?</summary>

Immer dann, wenn der Parameter an einer Stelle auftaucht, an der sein
**Vorzeichen oder Wert über die Anzahl der Lösungen entscheidet**:
unter einer Wurzel ($x = \pm\sqrt{t/3}$ – geht nur für $t \geq 0$), in
einem Nenner ($t \neq 0$?) oder als Faktor vor der höchsten Potenz
(ändert für $t < 0$ die Öffnungsrichtung). Dann jeden Fall getrennt zu
Ende denken.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Warum sortiert man für gemeinsame Punkte den Term nach $t$?</summary>

Ein Punkt liegt auf *allen* Kurven, wenn sein $y$-Wert **nicht von $t$
abhängt**. Nach dem Sortieren steht der komplette $t$-Einfluss in einem
Produkt wie $t(x^2 - 1)$ – und das verschwindet für jedes $t$ genau
dort, wo die Klammer null ist. Übrig bleibt ein fester Wert, den alle
Kurven teilen.
...
$f_t(x) = x^2 + t$. Berechne $f_2(3)$, $f_0(2)$ und
$f_{-3}(1)$.
...
Beschreibe die Wirkung des Parameters:
...
Durch welchen Punkt verlaufen **alle** Kurven der
...
Für welches $t$ verläuft $f_t(x) = x^2 + t$ durch
$P(2 \mid 1)$?
...
$g_t(x) = tx + 1$. Was haben alle Geraden der Schar
...
Leite nach $x$ ab: $f_t(x) = tx^3 - 3t^2x$
...
$f_t(x) = x^3 - 3tx^2 + 2$. Bestimme $f_t'$ und
$f_t''$.
...
Fehlersuche: Jemand leitet $f_t(x) = x^3 - tx$ so
...
Bestimme den Extrempunkt der Schar
...
$f_t(x) = x^3 - 3tx$ mit $t > 0$. Bestimme Hoch-
...
Untersuche $f_t(x) = x^3 + tx$ vollständig auf
...
Bestimme den Wendepunkt der Schar
...
Wie viele Nullstellen hat $f_t(x) = x^2 - t$ in
Abhängigkeit von $t$?
...
$f_t(x) = x^2 - 2tx + t^2$. Zeige, dass jede
...
Zeige, dass alle Kurven der Schar
...
Bestimme alle gemeinsamen Punkte der Schar
$f_t(x) = tx^2 + 1 - t$.
...
Bestimme den gemeinsamen Punkt aller Kurven der
...
Wurfschar $h_v(x) = x - \frac{10}{v^2}x^2$ (Graph
...
Berechne die maximale Wurfhöhe für $v = 10$ m/s
...
Mit welcher Abwurfgeschwindigkeit muss man
...
Medikamentenschar $K_d(t) = 0{,}1d \cdot t(t - 12)^2$
...
Das Medikament wirkt ab 10 mg/l. Welche
...
Für welche $t$ hat $f_t(x) = x^2 + t$ genau eine
Nullstelle?
...
Der Tiefpunkt von $f_t(x) = x^2 - tx$ ist
...
Für welchen Wert von $t$ **berührt** die Parabel
...
Ordne im Fallunterscheidungs-Plot der Erklärung
...
Begründe: **Alle** Kurven der Schar
...
a) Finde den Fehler: „$f_t(x) = tx^2 - t^2$, also
$f_t'(x) = 2tx - 2t$.“
b) Wahr oder falsch? „Alle Kurven der Schar $f_t(x) = x^2 + t$ haben
dieselbe Ableitung.“

<details>
<summary>Lösung zu Aufgabe 31</summary>

a) Beim Ableiten nach $x$ ist $t$ eine **Konstante** – und damit ist
auch $t^2$ eine Konstante: Ihre Ableitung ist $0$, nicht $2t$. Richtig:
$f_t'(x) = 2tx$. (Testweise $t = 5$: $5x^2 - 25 \to 10x$.)

b) **Wahr.** Der Parameter verschiebt hier nur in $y$-Richtung, und
Verschiebungen nach oben/unten ändern keine Steigung: Für jedes $t$ ist
$f_t'(x) = 2x$. Deshalb haben alle Kurven an derselben Stelle $x_0$
parallele Tangenten.
...
<Quiz fragen={[
  ...
  { frage: 'Verständnisfrage: Was ist die Ableitung von f_t(x) = x² − t² nach x?',
    antworten: ['2x − 2t', '2x', '2x − t²', '2x − 2tx'],
    richtig: 1, erklaerung: 't ist beim Ableiten nach x eine Konstante – also ist auch t² eine Konstante mit Ableitung 0.' },
  { frage: 'Verständnisfrage: Alle Kurven von f_t(x) = x² + t entstehen durch Verschieben in y-Richtung. Was folgt für die Ableitungen?',
    antworten: ['Sie unterscheiden sich um t', 'Alle Kurven haben dieselbe Ableitung 2x – Verschieben ändert keine Steigung', 'Nur für t = 0 gilt f′(x) = 2x', 'Die Ableitung ist 2x + t'],
    richtig: 1, erklaerung: 'Der konstante Summand t fällt beim Ableiten weg: An jeder Stelle x₀ haben alle Scharkurven parallele Tangenten.' },
]} />
