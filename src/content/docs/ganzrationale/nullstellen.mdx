...
<summary>Verständnisfrage: Warum <em>berührt</em> der Graph die $x$-Achse bei einer doppelten Nullstelle nur, statt sie zu schneiden?</summary>

Der Faktor $(x-2)^2$ ist ein Quadrat und damit **auf beiden Seiten von
$x = 2$ positiv** – er wechselt dort das Vorzeichen nicht. Also wechselt
auch $f(x)$ das Vorzeichen nicht: Der Graph kommt zur Achse herunter,
tippt sie an und dreht wieder um. Bei einer einfachen Nullstelle wechselt
der Linearfaktor das Vorzeichen – der Graph kreuzt.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Warum ist man nach der pq-Formel mit $z_1 = 4,\ z_2 = 1$ noch nicht fertig?</summary>

$z$ war nur eine **Hilfsvariable** – gefragt ist nach $x$! Jedes positive
$z$ steht für $x^2$ und liefert **zwei** $x$-Werte ($x = \pm\sqrt{z}$),
jedes negative $z$ keinen. Wer die Rücksubstitution vergisst, gibt
Lösungen einer ganz anderen Gleichung an.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Warum kommen als ganzzahlige Nullstellen überhaupt nur die Teiler des Absolutglieds infrage?</summary>

Setzt man eine ganze Zahl $x_0$ ein, enthält jeder Summand außer dem
Absolutglied den Faktor $x_0$: $x_0^3 - 2x_0^2 - 5x_0$ ist durch $x_0$
teilbar. Soll die ganze Summe $0$ ergeben, muss auch das Absolutglied
durch $x_0$ teilbar sein – sonst geht die Rechnung nie auf. Deshalb
testet man nur $\pm 1, \pm 2, \pm 3, \pm 6$.
...
Löse mit der pq-Formel: $x^2 - 7x + 12 = 0$
...
Löse durch Substitution: $x^4 - 13x^2 + 36 = 0$
...
Löse mit einer geeigneten Substitution:
$x^6 - 9x^3 + 8 = 0$
...
Begründe ohne vollständige Rechnung, dass
...
$f(x) = x^3 - 2x^2 - 5x + 6$ (Graph in der Erklärung)
...
Dividiere $\left(x^3 + 2x^2 - 5\right) : (x - 1)$.
...
Führe das Horner-Schema für $f(x) = x^3 - 7x + 6$ mit
...
Warum sind beim Raten ganzzahliger Nullstellen die
...
Nenne für jede Gleichung die passende Methode und löse:
...
$f(x) = x^4 - 4x^2$ (Graph in der Erklärung der Seite –
...
Berechne die Nullstellen von $f(x) = x^3 - 4x^2 + 4x$
...
Für welche Werte von $c$ hat $f(x) = x^3 - cx$ genau
**eine** Nullstelle?
...
Zeige ohne pq-Formel, dass $f(x) = x^4 + 2x^2 + 1$
...
Begründe: $x = 1$ ist für **jede** natürliche Zahl
...
$f(x) = x^3 - x^2 - 9x + 9$. Bestimme alle
...
a) „$x^4 - 5x^2 + 4 = 0$: Mit $z = x^2$ erhalte ich $z_1 = 4$ und
$z_2 = 1$. Die Gleichung hat also die Lösungen $4$ und $1$.“
b) „$(x-3)(x+1) = 5$: Nach dem Satz vom Nullprodukt ist $x - 3 = 5$ oder
$x + 1 = 5$.“

<details>
<summary>Lösung zu Aufgabe 31</summary>

a) Die **Rücksubstitution fehlt**: $z$ steht für $x^2$. Aus $x^2 = 4$
folgt $x = \pm 2$, aus $x^2 = 1$ folgt $x = \pm 1$ – die Gleichung hat
die vier Lösungen $\pm 1,\ \pm 2$, nicht „4 und 1“.

b) Der Satz vom Nullprodukt gilt **nur für $= 0$**: Ein Produkt kann auf
viele Arten 5 ergeben ($1 \cdot 5$, $2 \cdot 2{,}5$, …), aber null nur,
wenn ein Faktor null ist. Richtig: ausmultiplizieren, alles auf eine
Seite bringen ($x^2 - 2x - 8 = 0$) und dann lösen ($x_1 = 4$,
$x_2 = -2$).
...
<Quiz fragen={[
  ...
  { frage: 'Verständnisfrage: Der Graph berührt die x-Achse bei x = 2 nur, ohne sie zu kreuzen. Was folgt für den Term?',
    antworten: ['(x − 2) kommt in gerader Potenz vor (doppelte Nullstelle)', '(x − 2) kommt genau einmal vor', 'x = 2 ist gar keine Nullstelle', 'Das Absolutglied ist 2'],
    richtig: 0, erklaerung: 'Ein Quadrat wie (x − 2)² wechselt das Vorzeichen nicht – der Graph tippt die Achse nur an. Einfache Faktoren erzeugen echte Kreuzungen.' },
  { frage: 'Verständnisfrage: Warum darf man aus (x − 3)(x + 1) = 5 nicht „x − 3 = 5 oder x + 1 = 5“ folgern?',
    antworten: ['Weil man erst ausklammern muss', 'Weil der Satz vom Nullprodukt nur für Produkte gilt, die null ergeben', 'Weil 5 kein Teiler des Absolutglieds ist', 'Man darf das – die Lösungen stimmen'],
    richtig: 1, erklaerung: '5 lässt sich auf unendlich viele Arten als Produkt schreiben (1 · 5, 2 · 2,5 …). Nur bei = 0 muss einer der Faktoren null sein.' },
]} />
