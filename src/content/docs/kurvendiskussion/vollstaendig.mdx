...
<summary>Verständnisfrage: Warum ist die Reihenfolge des Schemas nicht beliebig?</summary>

Die Schritte bauen aufeinander auf: Die Symmetrie (2) halbiert alle
folgenden Rechnungen. Randverhalten (3) und Nullstellen (4) spannen das
Grundgerüst auf, in das Extrem- (5) und Wendepunkte (6) eingeordnet
werden. Und die Skizze (7) funktioniert nur als Schlusskontrolle, wenn
alle Ergebnisse schon vorliegen.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Warum <em>ist</em> eine doppelte Nullstelle automatisch ein Extrempunkt auf der $x$-Achse?</summary>

An einer doppelten Nullstelle berührt der Graph die Achse ohne
Vorzeichenwechsel: In einer Umgebung liegt er komplett oberhalb (oder
komplett unterhalb) und tippt die Achse nur im einen Punkt an. Der
Berührpunkt ist also lokal der tiefste bzw. höchste Punkt – ein Tief-
oder Hochpunkt mit $y = 0$.
...
<details class="verstaendnis">
<summary>Verständnisfrage: Warum liegt der Wendepunkt bei kubischen Funktionen <em>genau</em> in der Mitte zwischen Hoch- und Tiefpunkt?</summary>

Jede kubische Funktion ist **punktsymmetrisch zu ihrem Wendepunkt** –
der Graph sieht, um den Wendepunkt gedreht, wieder gleich aus. Also
müssen sich auch Hoch- und Tiefpunkt symmetrisch um ihn verteilen. Bei
höheren Graden gilt diese Kontrolle nicht mehr!
...
Prüfe auf Symmetrie: a) $x^4 - 8x^2 + 3$
b) $x^3 - 4x$  c) $x^3 + x^2$
...
$f(x) = x^3 - 4x$: Bestimme $y$-Achsenabschnitt und
alle Nullstellen.
...
Bestimme den Wendepunkt von $f(x) = x^3 - 3x^2 + 4$.
...
Gib die Monotonie-Intervalle von $f(x) = x^3 - 3x$
an.
...
Untersuche das Krümmungsverhalten von
$f(x) = x^3 - 6x^2$.
...
Zeige, dass $f(x) = x^3 - 3x^2 + 3x$ keinen
...
Bestimme den Wertebereich von $f(x) = x^4 - 2x^2$
...
Von einer Funktion dritten Grades sind bekannt:
...
Fehlersuche: In einer Klassenarbeit steht „$f'(2) = 0$,
...
Diskutiere vollständig: $f(x) = x^3 - 3x$
...
Diskutiere vollständig: $f(x) = x^3 - 6x^2 + 9x$
...
Diskutiere vollständig: $f(x) = -x^3 + 3x^2$
...
Diskutiere vollständig: $f(x) = x^4 - 4x^2$
...
Diskutiere vollständig: $f(x) = x^3 + 3x^2 - 4$
...
Diskutiere vollständig: $f(x) = 2x^3 - 6x$
...
Ordne die vier Terme den Graphen A–D zu:
...
Begründe für Graph C aus Aufgabe 21 **ohne
...
Lies am Ablese-Graphen (Beispiel 8) die
...
Am Ablese-Graphen berührt der Graph die $x$-Achse
...
Skizziere (beschreibe) den Graphen von $f'$ zum
...
Rekonstruiere den Funktionsterm des
...
Zeige rechnerisch, dass das Fieber nach 9 Stunden
...
Berechne die mittlere Änderungsrate der
...
Medikament $K(t) = 0{,}1t(t - 12)^2$ (Beispiel 7):
...
Ab einer Konzentration von 10 mg/l wirkt das
...
Laktatmodell $L(t) = 6t^2 - t^3$ ($0 \leq t \leq
...
Ein zweiter Proband hat die Laktatkurve
...
Begründe allgemein: Wenn ein Bestand durch eine
...
Fieberkurve: Ein fiebersenkendes Mittel soll
...
<details class="aufgabe">
<summary>Aufgabe 36 ⭐⭐</summary>

Begründe: Jede ganzrationale Funktion **dritten**
...
<details class="aufgabe">
<summary>Aufgabe 37 ⭐⭐⭐</summary>

Kann eine Funktion vierten Grades genau **einen**
...
<details class="aufgabe">
<summary>Aufgabe 38 ⭐⭐⭐</summary>

$f(x) = x^3 + bx$ mit beliebigem $b$. Zeige, dass
...
<details class="aufgabe">
<summary>Aufgabe 39 ⭐⭐</summary>

Wie viele Extrempunkte kann eine ganzrationale
...
<details class="aufgabe">
<summary>Aufgabe 40 ⭐⭐⭐</summary>

Vom Graphen von $f$ ist bekannt: $H(0 \mid 2)$
...
<details class="aufgabe">
<summary>Aufgabe 43 ⭐⭐⭐</summary>

Prüfe, dass $f(x) = \frac{1}{3}x^3 - 2x^2$ einen
...
<details class="aufgabe">
<summary>Aufgabe 44 ⭐⭐⭐</summary>

Erkläre, warum bei einer achsensymmetrischen
...
<details class="aufgabe">
<summary>Aufgabe 45 ⭐⭐⭐</summary>

Klassenarbeits-Simulation (45 min):
...
<details class="aufgabe">
<summary>Aufgabe 46 ⭐⭐ · Verständnisaufgabe</summary>

Beurteile ohne nachzurechnen:
a) „Meine Kurvendiskussion ergab einfache Nullstellen bei $x = 1$ und
$x = 3$ und den einzigen Extrempunkt bei $x = 5$.“ Kann das stimmen?
b) „$f(x) = x^4 + 2x^2 - 3$: Ich untersuche nur $x \geq 0$ und spiegele
alle Ergebnisse an der $y$-Achse.“ Ist das erlaubt?

<details>
<summary>Lösung zu Aufgabe 46</summary>

a) **Nein.** An einfachen Nullstellen wechselt $f$ das Vorzeichen: Bei
$x = 1$ kreuzt der Graph die Achse, bei $x = 3$ kreuzt er zurück.
Dazwischen muss er umkehren – zwischen 1 und 3 liegt zwingend ein
Extrempunkt. Irgendwo steckt ein Rechenfehler.

b) **Ja.** Der Term enthält nur gerade Exponenten ($x^4$, $x^2$, und
$-3 = -3x^0$), die Funktion ist also achsensymmetrisch zur $y$-Achse.
Jede Nullstelle, jeder Extrempunkt rechts hat ein exaktes Spiegelbild
links – die halbe Rechnung genügt.
...
<Quiz fragen={[
  ...
  { frage: 'Verständnisfrage: Einfache Nullstellen bei 1 und 3, aber angeblich kein Extremum dazwischen. Was folgt?',
    antworten: ['Das ist möglich, wenn f Grad 3 hat', 'Rechenfehler: Zwischen zwei Vorzeichenwechseln muss der Graph umkehren', 'Der Wendepunkt ersetzt das Extremum', 'Die Nullstellen sind falsch nummeriert'],
    richtig: 1, erklaerung: 'Bei x = 1 wechselt f z. B. von + nach −, bei x = 3 zurück – dazwischen muss ein Tiefpunkt liegen. Die Selbstkontrolle schlägt an.' },
  { frage: 'Verständnisfrage: Warum liegt der Wendepunkt kubischer Funktionen exakt zwischen Hoch- und Tiefpunkt?',
    antworten: ['Zufall, der meistens gilt', 'Weil kubische Graphen punktsymmetrisch zu ihrem Wendepunkt sind', 'Weil f″ eine Gerade ist, gilt es für alle Funktionen', 'Weil der Wendepunkt immer auf der y-Achse liegt'],
    richtig: 1, erklaerung: 'Die Punktsymmetrie zum Wendepunkt erzwingt gleiche Abstände zu H und T. Ab Grad 4 gibt es diese Garantie nicht mehr.' },
]} />
