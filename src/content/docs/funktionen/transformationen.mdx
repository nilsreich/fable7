...
<summary>Verständnisfrage: Warum bleiben bei $g(x) = a \cdot f(x)$ genau die Nullstellen liegen – und sonst (fast) kein Punkt?</summary>

Jeder $y$-Wert wird mit $a$ multipliziert. Ein Punkt bleibt nur dann, wo
er ist, wenn sein $y$-Wert das Multiplizieren unbeschadet übersteht – und
das gilt nur für $y = 0$: $a \cdot 0 = 0$. Alle anderen Punkte rücken von
der $x$-Achse weg ($|a| > 1$) oder zu ihr hin ($|a| < 1$).
...
<details class="verstaendnis">
<summary>Verständnisfrage: Bei $g(x) = f(2x)$ wird mit 2 multipliziert – warum wird der Graph trotzdem <em>schmaler</em> statt breiter?</summary>

Die 2 wirkt auf die **Eingabe**, nicht auf den Funktionswert: Schon bei
$x = 1$ berechnet $g$ den Wert $f(2)$ – alles, was $f$ erst bei 2 zeigt,
passiert bei $g$ schon bei 1. Der komplette Verlauf ist auf die halbe
Breite zusammengeschoben. Wie bei der $x$-Verschiebung gilt: Eingriffe
*innerhalb* der Klammer wirken „umgekehrt“.
...
<details class="verstaendnis">
<summary>Verständnisfrage: $2^x$ hat keine Nullstelle – warum hat $2^x - 4$ plötzlich eine?</summary>

$2^x$ liefert nur **positive** Werte, die $x$-Achse wird nie erreicht.
Schiebt man alles um 4 nach unten, liegen die Werte zwischen $-4$ und
$\infty$ – die 0 ist jetzt dabei: $2^x - 4 = 0$ genau bei $2^x = 4$, also
$x = 2$. Verschiebungen in $y$-Richtung können Nullstellen erzeugen oder
vernichten.
...
Beschreibe die Transformation der Normalparabel:
...
Gib den Term an: Die Normalparabel wird
...
Der Punkt $P(2 \mid 4)$ liegt auf der Normalparabel.
...
Im Vorher-Nachher-Bild zur $x$-Verschiebung (Erklärung)
...
Beschreibe alle Transformationen von
...
$g(x) = \dfrac{1}{x - 2} + 1$. Beschreibe die
...
$g(x) = 2^x - 4$ (Graph in der Erklärung).
...
$f(x) = x^3 - 3x$ hat die Nullstellen $0$ und
...
$g(x) = \sqrt{-x}$. Aus welcher Grundfunktion und
...
Bei $f(x) = x^2$ wird einmal **erst** um 2 nach oben
...
Begründe, dass zum Kombinations-Graphen der Erklärung
...
Der Graph von $f(x) = x^2 - 4$ wird um 1 nach oben
...
Multipliziere den Term aus Beispiel 3 aus:
$g(x) = (x - 2)^3 - 3(x - 2) + 1$.
...
Zeige durch quadratische Ergänzung, dass
...
Beschreibe $f(x) = 2\sin(x) + 1$ als Transformation
...
Welche der Transformationen lassen **alle**
...
Beschreibe die Transformation von $f(x) = 2^x$:
...
$f(x) = x^2$ ist achsensymmetrisch zur $y$-Achse. Gilt
...
Der Verlauf einer leichten Fieberkurve werde
...
Gib einen Funktionsterm an: eine nach unten
...
Finde und erkläre den Denkfehler:
a) „$f(x + 2)$ verschiebt den Graphen nach rechts – plus heißt ja
vorwärts.“
b) „Bei $g(x) = 3 \cdot f(x)$ wird jeder Punkt von der $x$-Achse
weggezogen, also verschieben sich auch die Nullstellen.“

<details>
<summary>Lösung zu Aufgabe 25</summary>

a) **Falsch.** $f(x+2) = f(x - (-2))$ verschiebt um $d = -2$, also nach
**links**: $g$ zeigt bei $x = -2$ schon das, was $f$ erst bei $0$ tut.
Innerhalb der Klammer wirkt alles „umgekehrt“.

b) **Falsch.** Gestreckt werden die $y$-Werte: Aus $f(x_0) = 0$ wird
$3 \cdot 0 = 0$. Punkte **auf** der $x$-Achse haben nichts, was gestreckt
werden könnte – die Nullstellen bleiben exakt liegen.
...
<Quiz fragen={[
  ...
  { frage: 'Verständnisfrage: Worin unterscheiden sich f(x) + 2 und f(x + 2)?',
    antworten: ['Gar nicht – Addition ist Addition', 'f(x) + 2 schiebt nach oben, f(x + 2) nach links', 'f(x) + 2 schiebt nach oben, f(x + 2) nach rechts', 'Beide strecken den Graphen'],
    richtig: 1, erklaerung: 'Außerhalb der Klammer wirkt die +2 auf die y-Werte (nach oben). In der Klammer wirkt sie auf die Eingabe – und zwar umgekehrt: nach links.' },
  { frage: 'Verständnisfrage: Warum hat 2ˣ − 4 eine Nullstelle, 2ˣ selbst aber nicht?',
    antworten: ['Weil die 4 eine Nullstelle bei x = 4 erzeugt', 'Weil 2ˣ nur positive Werte hat und erst die Verschiebung nach unten die 0 in den Wertebereich holt', 'Weil sich die Asymptote nicht ändert', '2ˣ hat doch eine Nullstelle bei x = 0'],
    richtig: 1, erklaerung: '2ˣ &gt; 0 für alle x. Nach der Verschiebung liegen die Werte in ]−4; ∞[ – die 0 wird bei 2ˣ = 4, also x = 2 erreicht.' },
]} />
...
- [ ] Ich kann bei gebrochenrationalen Funktionen Definitionslücken,
  Polstellen, Asymptoten und Nullstellen bestimmen.
  → [Gebrochenrationale Funktionen](../gebrochenrationale/#erklärung)
